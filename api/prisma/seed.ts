...
import { builtinPacks, DEFAULT_ACTIONS, type CharItem, type PackSeed } from './seed-data'
import { rarityFromTitle } from '../src/game/game.rarity'

const prisma = new PrismaClient()

function charTitle(item: CharItem): string {
  return typeof item === 'string' ? item : item[0]
}

function toCharacteristic(packageId: string, category: string, item: CharItem) {
  ...
  return {
    ...
    title,
    description,
    rarity: rarityFromTitle(title),
  }
}

async function upsertPack(pack: PackSeed) {
  ...
  const existingDisasters = new Set(
    (
  ...
  const existingBunkers = new Set(
    (
      await prisma.bunker.findMany({
        where: { packageId },
        select: { title: true },
      })
    ).map((b) => b.title),
  ...
  // Keep rarity in sync for builtin packs (deterministic from title).
  ...
  for (const row of packChars) {
    const next = rarityFromTitle(row.title)
    if (row.rarity !== next) {
      await prisma.characteristic.update({ where: { id: row.id }, data: { rarity: next } })
      rarityUpdates += 1
    }
  }
  if (rarityUpdates) console.log(`  ~rarity ${pack.slug}: ${rarityUpdates}`)

  const actions = pack.actions?.length ? pack.actions : DEFAULT_ACTIONS
  const existingActions = new Set(
    (
      await prisma.actionCard.findMany({
        where: { packageId },
        select: { title: true },
      })
    ).map((a) => a.title),
...
async function main() {
  ...
  console.log(
    `Seed ready: packages=${totals[0]}, disasters=${totals[1]}, bunkers=${totals[2]}, characteristics=${totals[3]}, actions=${totals[4]}`,
  )
}
