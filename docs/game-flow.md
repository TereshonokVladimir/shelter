...
- **Стратегия раскрытия** — *форма* раздачи квоты на раунды (не фиксированная длина партии):
  ...
  - `custom` — хост задаёт квоты по каждому раунду (сумма = 7)
...
При ничьей: короткие речи **только у кандидатов с равными голосами** (не весь круг), затем переголосование на то же число мест (`seats_needed`), обычно 1.
